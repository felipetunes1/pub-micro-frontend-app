...
export class RouteListComponent implements OnInit {
    ...
    if (route.remove) {
      this.dialog.open(MessageDialogComponent, {
        ...
        if (result) {
          ...
          this.router.navigateByUrl(`/generic-orchestrator/detail/${this.activateRouter.snapshot.params.id}/route`);
        }
      });
    } else {
      this.routeEditService.route = route;
  ...
  showXml() {
    this.router.navigateByUrl(`/generic-orchestrator/detail/${this.activateRouter.snapshot.params.id}/xml`);
  }
}
