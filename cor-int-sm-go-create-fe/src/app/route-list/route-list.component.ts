...
export class RouteListComponent implements OnInit {
  ...
  ngOnInit(): void {
    // this.routeEditService.create();
    console.log(this.routeEditService)
  }
    ...
    if (route.remove) {
      this.dialog.open(MessageDialogComponent, {
        ...
        if (result) {
          ...
          this.router.navigateByUrl(`/generic-orchestrator/new/route`);
        }
      });
    } else {
      this.routeEditService.route = route;
  ...
  showXml() {
    this.router.navigateByUrl(`/generic-orchestrator/new/xml`);
  }
}
